...
- [ ] [Architecture Guidelines](./ARCHITECTURE.md) - System architecture and design patterns (if applicable)
- [ ] [API Standards](./API_STANDARDS.md) - API design and implementation standards (if applicable)
- [ ] [Related Standards](./STANDARDS.md) - Any other relevant standards documentation (if applicable)
...
```
ProjectName/Features/ExampleFeature/
├── ExampleEndpoint.cs                       ✨ New - Main endpoint implementation
├── ExampleModels.cs                         ✨ New - Request/Response models
├── ExampleValidator.cs                      ✨ New - FluentValidation validator
└── ExampleMapper.cs                         ✨ New - Entity to DTO mapper

ProjectName.Tests.Integration/Tests/ExampleFeature/
├── ExampleEndpointTests.cs                  ✨ New - Integration tests
└── ExampleValidationTests.cs                ✨ New - Validation tests

ProjectName.Tests.Unit/ExampleFeature/
└── ExampleMapperTests.cs                    ✨ New - Unit tests for mapper
```
...
> - See [Testing Standards](./TESTING_STANDARDS.md) for behavioral testing guidance (if applicable)
> - Reference specialized testing docs when relevant for your project
...
- [ ] Add `private historyEntries: HistoryEntry[] = []` to store
- [ ] Create recordHistory method:
      `csharp
    public void RecordHistory(Item item) {
      this.historyEntries.Add(new HistoryEntry {
        Item = item,
